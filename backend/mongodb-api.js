...
app.get('/api/orders/search/:orderNumber', async (req, res) => {
  try {
    ...
    // const statusCollection = db.collection('production_status') // Supprimé
    // const itemsWithStatus = await Promise.all(items.map(async (item) => { // Supprimé
    //   const status = await statusCollection.findOne({ // Supprimé
    //     line_item_id: item.line_item_id // Supprimé
    //   }) // Supprimé
    //   // Supprimé
    //   return { // Supprimé
    //     ...item, // Supprimé
    //     production_status: status || { // Supprimé
    //       status: 'a_faire', // Supprimé
    //       production_type: null, // Supprimé
    //       assigned_to: null // Supprimé
    //     } // Supprimé
    //   }) // Supprimé
    // }) // Supprimé
    
    // Déterminer le type de production principal (le plus fréquent ou le premier trouvé)
    // let mainProductionType = null // Supprimé
    // const productionStatuses = itemsWithStatus // Supprimé
    //   .map(item => item.production_status) // Supprimé
    //   .filter(status => status && status.production_type) // Supprimé
    // // Supprimé
    // if (productionStatuses.length > 0) { // Supprimé
    //   // Compter les types de production // Supprimé
    //   const typeCounts = {} // Supprimé
    //   productionStatuses.forEach(status => { // Supprimé
    //     if (status.production_type) { // Supprimé
    //       typeCounts[status.production_type] = (typeCounts[status.production_type] || 0) + 1 // Supprimé
    //     } // Supprimé
    //   }) // Supprimé
    //   // Prendre le type le plus fréquent, sinon le premier // Supprimé
    //   if (Object.keys(typeCounts).length > 0) { // Supprimé
    //     mainProductionType = Object.entries(typeCounts).reduce((a, b) =>  // Supprimé
    //       typeCounts[a[0]] > typeCounts[b[0]] ? a : b // Supprimé
    //     )[0] // Supprimé
    //   } else { // Supprimé
    //     mainProductionType = productionStatuses[0]?.production_type || null // Supprimé
    //   } // Supprimé
    // } // Supprimé
    ...
    const orderWithProduction = {
      ...order,
      // production_type: mainProductionType, // Supprimé
      line_item_id: items[0].line_item_id,
      items: items,
      // all_production_statuses: productionStatuses // Supprimé
    }
    ...
    console.log(`🔍 Recherche commande ${orderNumber}:`, {
      ...
      // production_type: mainProductionType, // Supprimé
      items_with_status: items.map(item => ({
        line_item_id: item.line_item_id,
        product_name: item.product_name,
        // has_production_status: !!item.production_status, // Supprimé
        // production_type: item.production_status?.production_type, // Supprimé
        // status: item.production_status?.status // Supprimé
      }))
    })
...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    const ordersWithDetails = await Promise.all(orders.map(async (order) => {
      ...
      // const itemsWithStatus = await Promise.all(items.map(async (item) => { // Supprimé
      //   const status = await statusCollection.findOne({ // Supprimé
      //     order_id: order.order_id, // Supprimé
      //     line_item_id: item.line_item_id // Supprimé
      //   }) // Supprimé
      //   // Supprimé
      //   return { // Supprimé
...
app.get('/api/orders/production/:type', async (req, res) => {
  try {
    ...
    const assignedItems = await itemsCollection.find({
      // production_type: type // Supprimé
    }).toArray()
    ...
    if (assignedItems.length === 0) {
      ...
      for (const item of allItems) {
        // const existingStatus = await statusCollection.findOne({ // Supprimé
        //   order_id: item.order_id, // Supprimé
        //   line_item_id: item.line_item_id // Supprimé
        // }) // Supprimé
        
        // if (!existingStatus) { // Supprimé
        //   const productionType = determineProductionType(item.product_name) // Supprimé
        //   console.log(`📋 Dispatch automatique: ${item.product_name} -> ${productionType}`) // Supprimé
          
        //   if (productionType === type) { // Supprimé
        //     const productionStatus = { // Supprimé
        //       order_id: parseInt(item.order_id), // Supprimé
        //       line_item_id: parseInt(item.line_item_id), // Supprimé
        //       status: 'a_faire', // Supprimé
        //       production_type: productionType, // Supprimé
        //       assigned_to: null, // Supprimé
        //       created_at: new Date(), // Supprimé
        //       updated_at: new Date() // Supprimé
        //     } // Supprimé
            
        //     await statusCollection.insertOne(productionStatus) // Supprimé
        //     assignedItems.push(productionStatus) // Supprimé
        //   } // Supprimé
        // } // Supprimé
      }
    ...
    const ordersWithDetails = await Promise.all(assignedItems.map(async (item) => {
      const order = await db.collection('orders_sync').findOne({ order_id: item.order_id })
      // const item = await db.collection('order_items').findOne({ // Supprimé
      //   order_id: status.order_id, // Supprimé
      //   line_item_id: status.line_item_id // Supprimé
      // }) // Supprimé
...
app.post('/api/production/dispatch', async (req, res) => {
  try {
    ...
    const result = await db.collection('production_status').insertOne({
...
app.put('/api/production/redispatch', async (req, res) => {
  try {
    ...
    const existingStatus = await db.collection('production_status').findOne({
      order_id: parseInt(order_id),
      line_item_id: parseInt(line_item_id)
    })
    ...
    const result = await db.collection('production_status').updateOne(
      {
        order_id: parseInt(order_id),
        line_item_id: parseInt(line_item_id)
      },
...
async function dispatchItemToProduction(orderId, lineItemId, productName) {
  try {
    ...
    // const productionType = determineProductionType(productName) // Supprimé
