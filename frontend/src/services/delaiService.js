...
async function fetchWithRetry(url, options = {}, retries = 0) {
  ...
  const timeout = setTimeout(() => {
    /* log désactivé */
    controller.abort()
  }, options.timeoutMs || (import.meta.env.DEV ? 10000 : 8000)) // 10s local, 8s prod
...
class DelaiService {
  ...
  async chargerJoursFeriesAnnee(annee) {
    ...
    try {
      ...
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), import.meta.env.DEV ? 10000 : 8000) // 10s local, 8s prod
